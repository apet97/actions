...
class EventSchemaRegistryTest {
    ...
    void getSchema_unknownType_returnsOnlyMetaVars() {
        ...
        for (EventType type : EventType.values()) {
            ...
            assertEquals(4, metaCount, "Expected 4 meta vars for " + type);
            assertEquals(1, rawCount, "Expected 1 event.raw for " + type);
            // Total should be event-specific vars + 4 meta + 1 raw
            assertTrue(schema.size() >= 5, "Schema for " + type + " should have at least 5 vars (meta + raw)");
        }
    }
