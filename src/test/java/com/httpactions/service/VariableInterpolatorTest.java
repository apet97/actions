...
class VariableInterpolatorTest {
    ...
    private VariableInterpolator engine;

    @BeforeEach
    void setUp() {
        engine = new VariableInterpolator();
    }
    ...
    @Test
    void interpolate_urlEncode_true_encodesQueryValues() {
        JsonObject event = new JsonObject();
        event.addProperty("name", "hello world");

        // Query string values are URL-encoded
        String result = engine.interpolate("https://api.example.com?q={{event.name}}", event, Map.of(), true);
        assertTrue(result.contains("hello+world") || result.contains("hello%20world"),
                "Expected URL-encoded query value, got: " + result);
    }

    @Test
    void interpolate_urlEncode_true_doesNotEncodePathValues() {
        // Path values (before '?') are NOT encoded so {{meta.backendUrl}} works
        String result = engine.interpolate("{{meta.backendUrl}}/v1/projects",
                new JsonObject(), Map.of("backendUrl", "https://api.clockify.me/api"), true);
        assertEquals("https://api.clockify.me/api/v1/projects", result);
    }
    ...
    @Test
    void interpolate_bodySizeLimit_countsUtf8Bytes() {
        String multiByte = "č";
        int repeat = (1_048_576 / multiByte.getBytes(StandardCharsets.UTF_8).length) + 1;
        String template = multiByte.repeat(repeat);

        IllegalStateException exception = assertThrows(
                IllegalStateException.class,
                () -> engine.interpolate(template, new JsonObject(), Map.of(), false)
        );

        assertTrue(exception.getMessage().contains("bytes"));
    }
}
