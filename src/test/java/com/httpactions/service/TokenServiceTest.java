...
class TokenServiceTest {
    ...
    @Test
    void normalizeClaims_baseUrlAliases_mappedToBackendUrl() throws Exception {
        when(addonConfig.getKey()).thenReturn(ADDON_KEY);

        Map<String, Object> baseUrlClaims = validClaims();
        baseUrlClaims.remove("backendUrl");
        baseUrlClaims.put("baseUrl", "https://api.clockify.me/api/v1");

        Map<String, Object> baseURLClaims = validClaims();
        baseURLClaims.remove("backendUrl");
        baseURLClaims.put("baseURL", "https://api.clockify.me/api");

        Map<String, Object> baseUrlResult = tokenService.verifyAndParseClaims(createJwt(baseUrlClaims));
        Map<String, Object> baseURLResult = tokenService.verifyAndParseClaims(createJwt(baseURLClaims));

        assertNotNull(baseUrlResult);
        assertNotNull(baseURLResult);
        assertEquals("https://api.clockify.me/api", baseUrlResult.get("backendUrl"));
        assertEquals("https://api.clockify.me/api", baseURLResult.get("backendUrl"));
    }
