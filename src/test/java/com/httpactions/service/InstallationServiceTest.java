...
class InstallationServiceTest {
    ...
    void handleInstalled_storesInstallationAndWebhookTokens() {
        ...
        when(webhookTokenRepository.findByWorkspaceIdAndWebhookPath(eq("ws-1"), anyString()))
        ...
        ArgumentCaptor<WebhookToken> whCaptor = ArgumentCaptor.forClass(WebhookToken.class);
        verify(webhookTokenRepository, times(2)).save(whCaptor.capture());
        var savedTokens = whCaptor.getAllValues();
    ...
    @Test
    void handleInstalled_missingWorkspaceId_returnsGracefully() {
        String body = """
                {
                  "addonId": "addon-1",
    ...
    @Test
    void handleInstalled_missingAuthToken_returnsGracefully() {
        String body = """
                {
                  "workspaceId": "ws-1",
                  "addonId": "addon-1"
    ...
    @Test
    void handleDeleted_missingWorkspaceId_returnsGracefully() {
        String body = """
                {
                  "addonId": "addon-1"
                }
    ...
    @Test
    void handleStatusChanged_unknownStatus_ignoresGracefully() {
        String body = """
                {
                  "workspaceId": "ws-1",
                  "status": "SUSPENDED"
        ...
        // Status should remain ACTIVE since SUSPENDED is unknown
        assertEquals(InstallationStatus.ACTIVE, existing.getStatus());
        verify(installationRepository, never()).save(any());
    }

    @Test
    void handleStatusChanged_missingWorkspaceId_returnsGracefully() {
        String body = """
                {
                  "status": "INACTIVE"
                }
    ...
    @Test
    void handleStatusChanged_missingStatus_returnsGracefully() {
        String body = """
                {
                  "workspaceId": "ws-1"
                }
