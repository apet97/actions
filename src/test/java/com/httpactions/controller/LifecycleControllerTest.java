...
class LifecycleControllerTest {
    ...
    private static final String INSTALLED_BODY_MISSING_WORKSPACE = """
            {"addonId":"addon-1","authToken":"tok-123","webhooks":[]}""";
    ...
    @Test
    @DisplayName("installed with malformed lifecycle payload returns 400")
    void installed_serviceRejectsPayload_returns400() throws Exception {
        when(tokenService.verifyAndParseClaims(VALID_TOKEN)).thenReturn(validClaims());
        doThrow(new IllegalArgumentException("INSTALLED payload missing required fields"))
                .when(installationService).handleInstalled(INSTALLED_BODY);
    ...
    @Test
    @DisplayName("installed with workspace mismatch returns 401")
    void installed_workspaceMismatch_returns401() throws Exception {
        Map<String, Object> claims = validClaims();
        claims.put("workspaceId", "different-workspace");
    ...
    @Test
    @DisplayName("installed with missing workspace in body returns 401")
    void installed_missingWorkspaceInBody_returns401() throws Exception {
        when(tokenService.verifyAndParseClaims(VALID_TOKEN)).thenReturn(validClaims());

        mockMvc.perform(post("/lifecycle/installed")
                        .header(LIFECYCLE_TOKEN_HEADER, VALID_TOKEN)
                        .content(INSTALLED_BODY_MISSING_WORKSPACE)
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isUnauthorized());
    ...
    @Test
    @DisplayName("installed with missing workspace in token claims returns 401")
    void installed_missingWorkspaceInClaims_returns401() throws Exception {
        Map<String, Object> claims = validClaims();
    ...
    @Test
    @DisplayName("status-changed with malformed payload returns 400")
    void statusChanged_serviceRejectsPayload_returns400() throws Exception {
        when(tokenService.verifyAndParseClaims(VALID_TOKEN)).thenReturn(validClaims());
        doThrow(new IllegalArgumentException("Unknown installation status: SUSPENDED"))
                .when(installationService).handleStatusChanged(STATUS_CHANGED_BODY);
