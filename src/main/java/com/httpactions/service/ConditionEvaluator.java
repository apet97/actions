...
public class ConditionEvaluator {
     ...
     *
     * @param conditionsJson JSON array of success conditions (from Action.successConditions), or null
    ...
    public boolean evaluateSuccessConditions(String conditionsJson, TestResult result) {
        ...
        try {
            conditions = GsonProvider.get().fromJson(conditionsJson, JsonArray.class);
        } catch (Exception e) {
            log.warn("Failed to parse success conditions JSON, falling back to default: {}", e.getMessage());
            return result.isSuccess();
        }
        ...
        for (JsonElement element : conditions) {
            if (!element.isJsonObject()) {
                log.warn("Skipping malformed condition element (not a JSON object)");
                return false;
            }

            JsonObject condition = element.getAsJsonObject();
            String operator = getStringField(condition, "operator");
            String value = getStringField(condition, "value");

            if (operator == null) {
                log.debug("Skipping success condition with null operator");
                continue;
            }

            boolean passed = evaluateSingleSuccessCondition(operator, value, result);
            if (!passed) {
                log.debug("Success condition failed: operator={}, value={}, status={}", operator, value, result.getResponseStatus());
                return false;
            }
        }
     ...
     *
     * @param conditionsJson JSON array of execution conditions (from Action.conditions), or null
    ...
    public boolean evaluateExecutionConditions(String conditionsJson, JsonObject event) {
        ...
        try {
            ...
            log.warn("Failed to parse execution conditions JSON, defaulting to execute: {}", e.getMessage());
            return true;
        }
        ...
        for (JsonElement element : conditions) {
            ...
            JsonObject condition = element.getAsJsonObject();
            String field = getStringField(condition, "field");
            String operator = getStringField(condition, "operator");
            String value = getStringField(condition, "value");

            if (operator == null) {
                log.debug("Skipping execution condition with null operator");
                continue;
            }
    ...
    private boolean evaluateSingleExecutionCondition(String field, String operator, String value, JsonObject event) {
        ...
        return switch (operator) {
            ...
            case "not_equals" -> fieldValue == null || !fieldValue.equals(value);
